...
class ChatBController extends AbstractController
{
     ...
     */
    public function message(ChallengeRepository $challengeRepository): Response
    {
        ...
        $botman->hears('how can i participate', function (BotMan $bot) {
            $bot->reply('You only need to select a challenge, fill the form by writing a description and selecting an image and submit it.');
        });

        $botman->hears('how can i change my participation', function (BotMan $bot) {
            $bot->reply('If you not passed de limit date, you just need to fill the same form of the add and will be changed .');
        });

        $botman->hears('Is there any challenge today', function (BotMan $bot) {
                $bot->reply('you can check the calendar');
        });
        ...
        return new Response($botman->listen() ?? NULL) ;
    }
